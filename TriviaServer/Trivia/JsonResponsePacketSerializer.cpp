...
std::vector<json> JsonResponsePacketSerializer::createJsonRoomArray(std::vector<RoomData> rooms)
{
	...
	{
		jrooms.push_back({ {"id", rooms[i].id}, {"name", rooms[i].name}, {"maxPlayers", rooms[i].maxPlayers}, {"questionCount", rooms[i].questionCount}, {"timePerQuestion", rooms[i].timePerQuestion}, {"roomState", rooms[i].roomState} });
	}
	return jrooms;
}
